...
#What if I made this into an Animorphs themed game? Acquiring morphs, becoming a nothlit, etc. 
#Morphs can be controlled by taking a certain amount of turns and consuming a finite resource, and/or some sort of cooldown system. 
#For instance, reverting to human (Andalite?) could cost all your mana, forcing you to wait before you morph again. 
#Stronger morphs could cost more mana. Morphs could be implemented as components. This gives the possibility of morphing enemies too!
#Unlike the books, I could limit the amount of morphs the player character can 'remember' at one time.
#When in a form with hands, you can pick up items from enemies and the environment to aid your quest. These are meant to be effective but disposable. Limit ammo, etc.
#Items must be balanced to encourage morphing; forms with hands must be balanced as well.
#A timer item could be included; if your character doesn't have one, your will have to estimate your morph time. Probably need a warning of some kind.
#Make it so that hp only regenerates when human; this will encourage morphing rather than staying in the most powerful form. Alternatively, I could make it so that morphing is the only way to heal period. 
#Implement hunger, and make morphing have a hunger cost.
#Morph differentiation: enhanced senses, special attacks. In fact, why not have the main point of each morph to have one special ability?
#The quest can remain essentially the same, just replace the dungeon with Yeerk Base, Sanctum with Yeerk pool, and the Amulet with Kandrona Source.
#Diets! Different morphs can have different diets/hunger rates, which is a nice new spin on this old hunger mechanic.
...
class Object:
	def __init__(self, x, y, char, name, color, blocks = False, always_visible = False, fighter = None, morph = None, ai = None, item = None, equipment = None):
		self.x = x
		self.y = y
		self.char = char
		self.name = name
...
class Cow:
	def __init__(self, name = "cow", hp_modifier = 2):
		self.name = name
		self.hp_modifier = hp_modifier
	
	def ability(self):
		message("You moo mournfully.")
	...
	if game_state == 'playing':
		...
		#testing for other keys
			...
			if key_char == "r":
				#use morph ability
				if player.morph:
					player.morph.ability()
				else:
					message("You don't have a special ability.")
...
def make_sanctum():
	...
	message("Welcome to the Yeerk Pool! The Kadndrona Source is here somewhere...", libtcod.yellow)
	#the stairs
	up_stairs = Object(player.x, player.y, "<", "up stairs", libtcod.white, always_visible = True)
	...
	monster = Object(altar_x, altar_y, '&', "visser one", libtcod.dark_flame, blocks = True, fighter = fighter_component, ai = ai_component)
	objects.append(monster)
	#the Amulet
	equipment_component = Equipment(slot = 'pocket')
	amulet = Object(altar_x, altar_y, "*", "kandrona source", libtcod.azure, equipment = equipment_component)
	objects.append(amulet)
	send_to_back(amulet)
	#the altar
	altar = Object(altar_x, altar_y, "_", "complicated control panel", libtcod.darkest_gray, always_visible = True)
...
def render_all():
	...
	for (line, color) in game_msgs:
		...
		y += 1
	render_bar(1, 1, BAR_WIDTH, 'Vitality', player.fighter.hp, player.fighter.max_hp, libtcod.light_red, libtcod.darker_red)
	render_bar(1, 2, BAR_WIDTH, 'Stamina', player.fighter.hp, player.fighter.max_hp, libtcod.light_blue, libtcod.darker_blue)
	#display the player's current form
	#if you have the timer, you can track the time remaining in the form of a bar
	if False:
		render_bar(1, 4, BAR_WIDTH, 'Human', player.fighter.max_hp, player.fighter.max_hp, libtcod.black, libtcod.black)
	else:
		if player.morph:
			libtcod.console_print_ex(panel, 1, 4, libtcod.BKGND_NONE, libtcod.LEFT, "Current form: " + str(player.morph.name))
		else:
			libtcod.console_print_ex(panel, 1, 4, libtcod.BKGND_NONE, libtcod.LEFT, "Current form: human")
	#display dungeon level
	libtcod.console_print_ex(panel, 1, 5, libtcod.BKGND_NONE, libtcod.LEFT, 'Current level ' + str(dungeon_level))
...
def place_objects(room):
	...
	item_chances['lightning'] = 15
	item_chances['fireball'] = 15
	item_chances['confuse'] = 15
	#equipment
	item_chances['sword'] = 10
	item_chances['battleaxe'] = 5
	item_chances['shield'] = 5
	item_chances['helmet'] = 10
	item_chances['leather_armor'] = 10
	item_chances['chain_armor'] = 5
	#nothing
	item_chances['nothing'] = 5
	...
	for i in range(num_items):
		...
		if not is_blocked(x, y):
			...
			elif choice == "leather_armor":
				equipment_component = Equipment(slot = 'body', defense_bonus = 2)
				item = Object(x, y, '[', 'leather armor', libtcod.darker_orange, equipment = equipment_component)
...
def new_game():
	global player, inventory, game_msgs, game_state, dungeon_level, player_level, floors, stash, available_morphs
	#make player
	...
	morph_component = Cow()
	available_morphs.append(morph_component)
	#make messages
	game_msgs = []
...
def prev_level():
	...
	if dungeon_level == 1:
		if get_equipped_in_slot('neck'):
			message('You win!', libtcod.azure)
		else:
			message('You cannot leave without retrieving the Kandrona Source! The fate of the human race depends upon you!', libtcod.red)
	else:
...
def player_morph():
    #allows the player to morph
	if player.morph:
		#if the player is currently morphed
		choice = menu("Morph back to human?", ["Yes", "No"], INVENTORY_WIDTH)
		if choice == 0:
			player.morph = None
			message("You return to human form.")
	else:
		...
		if len(available_morphs) > 0:
			...
			for morph in available_morphs:
				text = morph.name
				options.append(text)
			choice = menu("What would you like to morph into?", options, INVENTORY_WIDTH)
			player.morph = available_morphs[choice]
			message("You feel a change coming over you. You become a " + str(available_morphs[choice].name) + "!")
		else:
			message("You haven't acquired any morphs!", libtcod.red)
...
def save_game():
	...
	file['available_morphs'] = available_morphs
...
def load_game():
	...
	global map, objects, player, up_stairs, stairs, inventory, available_morphs, game_msgs, game_state, dungeon_level, player_level, floors, stash
	...
	available_morphs = file['available_morphs']
