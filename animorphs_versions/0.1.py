...
#When in a form with hands, you can pick up items from enemies and the environment to aid your quest. These are meant to be effective but disposable.
...
#Make it so that hp only regenerates when human; this will encourage morphing rather than staying in the most powerful form.
#Alternatively, I could make it so that morphing is the only way to heal period. 
...
def render_all():
	...
	if False:
		render_bar(1, 3, BAR_WIDTH, 'Human', player.fighter.max_hp, player.fighter.max_hp, libtcod.black, libtcod.black)
	else:
		if player.morph:
			libtcod.console_print_ex(panel, 1, 3, libtcod.BKGND_NONE, libtcod.LEFT, "Current form: " + str(player.morph.name))
		else:
			libtcod.console_print_ex(panel, 1, 3, libtcod.BKGND_NONE, libtcod.LEFT, "Current form: human")
