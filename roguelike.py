...
#TO DO: messages telling you what's in the same tile as you, "nothing to pick up" message. 
#For spellcasters: emulate crawl's spell picking system. Create a spell picker function, that receives an array of possible spell choices. Create a separate registry of all spells. Perhaps it would be best to assign an ID to each spell?

#KNOWN BUGS: cannot pickup items (has something to do with the second "break")
...
class BasicSpellcaster:
	#component for a basic arcane spellcaster monster
		...
		if libtcod.map_is_in_fov(fov_map, monster.x, monster.y):
			if monster.distance_to(player) >= 2:
				if libtcod.random_get_int(0, 0, 1) == 1:
					...
					self.cast_spell(self.caster_level)
			elif player.fighter.hp > 0:
				if libtcod.random_get_int(0, 0, 1) == 1:
					monster.fighter.attack(player)
				else:
	...
	def cast_spell(self, level):
		message('The ' + self.owner.name + ' screams incoherently.', libtcod.dark_violet)
...
class ConfusedMonster:
	...
	def take_turn(self):
		if self.num_turns > 0:
			...
			message('The ' + self.owner.name + ' seems to have regained his bearings.', libtcod.red)
			...
			if key_char == "g":
				...
				for object in objects:
					...
					if object.x == player.x and object.y == player.y and object.item:
						...
						break
					#else:
					#	message('There is nothing to pick up here.', libtcod.white)
					#	break
			if key_char == "i":
				#show inventory
...
def place_objects(room):
	...
	monster_chances['orcish shaman'] = 80
	...
	for i in range(num_monsters):
		...
		if not is_blocked(x, y):
			...
			elif choice == "orcish shaman":
				ai_component = BasicSpellcaster(2)
				fighter_component = Fighter(hp = 10, defense = 0, power = 4, xp = 50, death_function = monster_death)
				monster = Object(x, y, 'o', "orcish shaman", libtcod.darker_blue, blocks = True, fighter = fighter_component, ai = ai_component)
			else:
				pass
			objects.append(monster)
...
def prev_level():
	...
	if dungeon_level == 1:
		if get_equipped_in_slot('neck'):
			...
			message('You cannot leave without retrieving the Amulet!', libtcod.red)
	else:
